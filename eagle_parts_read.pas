...
********************************************************************************
*
*   Subroutine EAGLE_PARTS_READ (FNAM, TNAM, GNAM, LIST, STAT)
*
*   Read the XXX_parts.csv file written by the BOM ULP.  This file contains the
*   list of parts in a schematic/board, one per line.
*
*   FNAM is the name of the CSV file to read.  The ".csv" file name suffix may
*   be omitted from FNAM.  The BOM ULP names the file XXX_parts.csv, where XXX
*   is the name of the board.
*
*   TNAM is returned the full tree name of the CSV file actually read.
*
*   GNAM is the generic name of the CSV file.  When the file name ends in
*   "_parts.csv", then that part is removed from the file name to make the
*   generic name.  Otherwise, only the .CSV file name suffix is removed.  When
*   the file is generated by the BOM ULP, then GNAM is the board name.
*
*   LIST is the list of parts to add to.  One part will be added for each
*   content line of the CSV file.
*
...
*   by the BOM ULP.  This routine returns without error and without reading the
...
{
*   Open the CSV file and make derived pathnames.
}
  csv_in_open (fnam, cin, stat);       {open the CSV input file}
  if sys_error(stat) then return;

  string_copy (cin.conn.tnam, tnam);   {return the full input file treename}
  string_generic_fnam (                {return the generic name (board name)}
    cin.conn.gnam, '_parts'(0), gnam);
{
...
{
*   The input file seems valid based on the field names in the first line.
*
